...
int main(int argc, char** argv) {
    ...
    do {
        retour = recvfrom(soc, &donneeRecue, sizeof (int), 0, (struct sockaddr *) &infoClient, &tailleInfoClient);
        if (retour == -1) {
            printf("pb envoie : %s \n", strerror(errno));
            exit(errno);
        }
    ...
    printf("message du client %s : %d", inet_ntoa(infoClient.sin_addr), donneeRecue);
