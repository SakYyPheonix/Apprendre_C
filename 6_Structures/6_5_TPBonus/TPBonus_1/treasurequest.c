...
void afficherPersonnage(typePersonnage *perso){
    ...
    printf("Nom : %s\nPrenom : %s\nAge : %ud\nTaille : %2f\nPoids : %2f\nResistance : %d\nPoints de vie : [%d/%ud]\nNiveau : %ud\nExperience dans le niveau : %ud/200\n", perso->nom, perso->prenom, perso->age, perso->taille, perso->poids, perso->resistancePhysique, perso->pv, perso->pvMax, perso->niveau, perso->experience);
...
void faireUnDuel(typePersonnage *j1, typePersonnage *j2) {
    ...
    j1->experience = j1->experience + exp;
    while(j1->experience >= 200){
        j1->experience = j1->experience - 200;
        j1->niveau++;   
    }
...
int calculDegats(typePersonnage *att, typePersonnage *def){
    int degats;
    degats = calculAttaque(att) - calculDefense(def);
    if(degats<=0){
        degats = 1;
    }
    return degats;
