...
void afficherMenu(int nbParcours) {
    if (nbParcours > 0) {
        printf("[J] Jouer un parcours\n[D] Jouer un parcours avec 2 joueurs\n[V] Afficher un parcours\n[C] Creer un parcours\n[Q] Quitter\n");
    } else {
...
void afficherInfoTrou(typeTrou unTrou) {
    printf("----------\n");
    printf("INFO TROUS\n\nnumero du trous : %d\ndistance : %f\nvaleur du par : %d\n nb de coup maximum : %d\n", unTrou.numero, unTrou.distance, unTrou.par, unTrou.nbMaxCoups);
    printf("----------\n");
}
...
void sauvegarderParcours(typeParcours *tab[], int nbParcours) {
    ...
    fchSave = fopen(FICHIER_PARCOURS, "w+");
    if (fchSave == NULL) {
    ...
    for (i = 0; i < nbParcours; i++) {
        retour = fwrite(tab[i], sizeof (typeParcours), 1, fchSave);
        if (retour != 1) {
            printf("erreur n%d : %s", errno, strerror(errno));
            exit(errno);
        }
...
void jouerParcoursADeux(typeParcours *p) {
    ...
    printf("difficulte du joueur 1 [0-infernale .. 5-tres facile]");
    scanf("%d", &sc[0]->difficulte);
    printf("difficulte du joueur 1 [0-infernale .. 5-tres facile]");
    scanf("%d", &sc[1]->difficulte);
    niveau[0] = 1.0 + 3.0 * (float) sc[0]->difficulte;
    niveau[1] = 1.0 + 3.0 * (float) sc[1]->difficulte;
    // recopie des donnees du parcours fait par le joueur
    ...
    for (i = 0; i < p->nbTrou; i++) {
        printf("\n%s a vous de jouer\n\n", sc[0]->nomJoueur);
        afficherInfoTrou(p->listeTrous[i]);
        sc[0]->points[i] = jouerTrou(p->listeTrous[i], niveau[0]);
        printf("\n%s a vous de jouer\n\n", sc[1]->nomJoueur);
        afficherInfoTrou(p->listeTrous[i]);
        sc[1]->points[i] = jouerTrou(p->listeTrous[i], niveau[1]);
    }
